...
- (instancetype)initWithSuperView:(UIView*)superView  andY:(CGFloat)offY {
    
    
    if (self = [super initWithFrame:CGRectMake(0.0, superView.frame.size.height+offY, superView.frame.size.width, kHooDatePickerHeight)]) {
...
-(void)show {
    ...
    [UIView animateWithDuration:kHooDatePickerAnimationDuration delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        self.frame = CGRectMake(self.frame.origin.x, self.y- kHooDatePickerHeight, self.frame.size.width, self.frame.size.height);
...
-(void)show:(CGFloat )offY{
    ...
    if (_isInitialized == NO) {
        self.frame = CGRectMake(self.frame.origin.x, _superView.frame.size.height + offY, self.frame.size.width, self.frame.size.height);
        _isInitialized = YES;
    }
...
- (UIView *)headerView {
    if (!_headerView) {
        ...
        [sureButton setTitleColor:[UIColor colorWithRed:26.0/255.0 green:126.0/255.0 blue:248.0/255.0 alpha:1]forState:UIControlStateNormal];
